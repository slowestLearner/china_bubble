...
# import data
stock_character = pd.read_parquet(os.path.join(RESULTS_DIR, "stock_character/stock_processed"))
...
ret_201415['cum_ret'] = ret_201415.groupby(['SEC_CODE'])['ret'].transform(lambda x : (x+1).cumprod()-1) * 100

max_ret = ret_201415.loc[ret_201415.groupby(['SEC_CODE'])['cum_ret'].idxmax()][['SEC_CODE','TRADE_DATE','cum_ret']]
...
bubble_size = kk.loc[kk.groupby(['SEC_CODE'])['cum_ret'].idxmin()][['SEC_CODE','TRADE_DATE','cum_ret','max_cum_ret_date','max_cum_ret','INDUSTRYCODE','INDUSTRYNAME']]
bubble_size.columns =['SEC_CODE', 'min_cum_ret_date', 'min_cum_ret','max_cum_ret_date','max_cum_ret','IndustryCode','IndustryName']
kk = bubble_size.groupby(['min_cum_ret_date']).count()[['SEC_CODE']]

bubble_size['crash'] = (1+bubble_size['min_cum_ret']/100)/(1+bubble_size['max_cum_ret']/100)-1
bubble_size['crash'] = - bubble_size['crash'] * 100
bubble_size = bubble_size.rename(columns={"max_cum_ret": "runup"})
bubble_size = bubble_size.reindex(columns=['SEC_CODE', 'min_cum_ret_date', 'min_cum_ret','max_cum_ret_date','runup','crash','IndustryCode','IndustryName'])

### Run-up and Crash ###
with open(os.path.join(TABLES_DIR, "summary_stat_boom_bust.tex"), "w") as f: f.write(bubble_size[['runup', 'crash']].describe().to_latex(float_format="%.2f"))
...
retail_hold_stock['retail_share'] = retail_hold_stock['Retail_Close_Value']/(retail_hold_stock['Retail_Close_Value']+retail_hold_stock['Institution_Close_Value'])

# import stock characteristics
characteristics = stock_character.copy()
stocks_MP_retail = pd.read_parquet(os.path.join(DATA_DIR, "additional_results_from_return_chasing_paper/MP_Decomposition/Stocks_MP"))
...
characteristics = characteristics.merge(stocks_MP_retail, on=['SEC_CODE','TRADE_DATE'], how = 'left')
characteristics = pd.merge(characteristics, retail_hold_stock, on=['SEC_CODE','TRADE_DATE'], how='left')
...
winsor_list = ['PE','PB','Profitability', 'Asset_YoY', 'Book_YoY', 'dividend_be','ln_be']
characteristics = characteristics.groupby(['TRADE_DATE']).apply(lambda month: winsor_month_data(month, winsor_list))
...
zscore_cols = ['PE_win','PB_win','retail_share','turnover','MP_retail','beta','Book_YoY_win','Asset_YoY_win', 'ln_be','ln_be_win','Profitability_win','dividend_be_win','ipo_gap', 'PE','PB','Book_YoY','Asset_YoY','Profitability','dividend_be','beta_b2014','SOE', 'past_cumret_12']
...
characteristics[zscore_cols_name] = (
    characteristics
    .groupby('TRADE_DATE')[zscore_cols]
    .transform(lambda x: stats.zscore(x, nan_policy='omit'))
)

### Identify big V.S. small bubbles ###

bubble_size['runup_sort'] = pd.qcut(bubble_size['runup'], 2, labels=False,duplicates='drop') + 1
bubble_size['crash_sort'] = pd.qcut(bubble_size['crash'], 2, labels=False,duplicates='drop') + 1

bubble_size['big_bubble'] = 0
bubble_size['small_bubble'] = 0
bubble_size.loc[(bubble_size['runup_sort'] == 2) & (bubble_size['crash_sort'] == 2), 'big_bubble'] = 1
bubble_size.loc[(bubble_size['runup_sort'] == 1) & (bubble_size['crash_sort'] == 1), 'small_bubble'] = 1
...
bubble_diff['sector_cum_ret'] = bubble_diff.groupby(['SEC_CODE'])['ind_ret'].transform(lambda x : (x+1).cumprod()-1) * 100
bubble_diff['ret_cum'] = bubble_diff.groupby(['SEC_CODE'])['ret'].transform(lambda x : (x+1).cumprod()-1) * 100
...
temp = temp[(temp.Month_End >= pd.to_datetime(20140701, format='%Y%m%d')) & (temp.Month_End <= pd.to_datetime(20160131, format='%Y%m%d'))]

example = temp[temp.SEC_CODE.apply(lambda x: x in set([600005, 600019]))]
example = example.groupby('SEC_Abbr')[['retail_share','beta_b2014',
        'Book_Value','Profitability','dividend_be']].mean()
example['Book_Value'] = example['Book_Value'] / 1e9
example[['retail_share','Profitability','dividend_be']] = example[['retail_share','Profitability','dividend_be']] * 100
example = example[['beta_b2014','Book_Value','retail_share','Profitability','dividend_be']]
with open(os.path.join(TABLES_DIR, "example.tex"), "w") as f: f.write(example.to_latex(float_format="%.2f"))

temp = temp.set_index(['SEC_CODE','Month_End'])

dep_list= ['retail_share_z','beta_z','Book_YoY_z', 'ln_be_z','SOE_z','Profitability_z','dividend_be_z','ipo_gap_z']
name_list = ['RetailShare','CAPM Beta','Investment', 'Size','SOE','Profitability','Dividend','Age']
...
coef_df = pd.DataFrame(list(zip(beta_list, err_list, dep_list)), columns =['coef', 'err','varname'])
...
temp_prebubble = temp_prebubble[(temp_prebubble.Month_End < pd.to_datetime(20140701, format='%Y%m%d'))]
...
coef_df_pre = pd.DataFrame(list(zip(beta_list, err_list, dep_list)), columns =['coef', 'err','varname'])
...
i=0
X = base_x + width*i
ax.bar(X, coef_df_pre['coef'],yerr=coef_df_pre['err'],color='none',ecolor=color_list[i])
## axis labels
...
ax.scatter(x=X, 
            ...
            y=coef_df_pre['coef'], color=color_list[i])
# bar 2: during-bubble        
i=1
X = base_x + width*i
...
ax.scatter(x=X, 
            marker=marker_list[i], s=120, 
            y=coef_df['coef'], color=color_list[i])
...
legend_elements = [Line2D([0], [0], marker=marker_list[0], label='Pre-Bubble', color = color_list[0], markersize=10), Line2D([0], [0], marker=marker_list[1], label='Bubble', color = color_list[1], markersize=10)]
ax.legend(handles=legend_elements, loc=0, prop={'size': 12}, labelspacing=1.2)
fig.savefig(os.path.join(FIGURES_DIR, "difference_in_characteristics.png"), dpi=300, bbox_inches='tight') 