...
stock_character = pd.read_parquet(os.path.join(RESULTS_DIR, "stock_character/stock_character_20220827"))

# set stocks listed in one year as outside assets
stock_character['ipo_gap'] = pd.to_datetime(
    stock_character['Month_End'], format='%Y%m%d') - pd.to_datetime(
        stock_character['IPO_DATE'], format='%Y%m%d')
...
stock_character = stock_character[stock_character['Month_End'].between(20110000, 20199999)]

stock_character['SOE'] = stock_character.StockAttr.isin(['地方国有企业', '中央国有企业']).astype('int')

# import me_iv
me_IV = pd.read_parquet(os.path.join(RESULTS_DIR, "stock_character/me_iv"))

me_IV = me_IV[me_IV.TYPE2 == 'SNQR']

me_IV = me_IV[['SEC_CODE','me_iv_equal','me_iv_weight','Month_End']]
me_IV.columns = ['SEC_CODE','me_iv1','me_iv2','Month_End']

me_IV['me_iv1'] = np.where(me_IV['me_iv1']<=1, 1, me_IV['me_iv1']) # replace me_iv1 < 1 with 1
me_IV['me_iv1_log'] = np.log(me_IV['me_iv1'] )
me_IV['me_iv1_log_square'] = me_IV['me_iv1_log'] * me_IV['me_iv1_log']

me_IV['me_iv2'] = np.where(me_IV['me_iv2']<=1, 1, me_IV['me_iv2']) # replace me_iv2 < 1 with 1
...
na_cols =  ['ret_lag_1','ret_lag_2', 'ret_lag_3', 'ret_lag_4', 'ret_lag_5', 'ret_lag_6','ret_lag_7', 'ret_lag_8', 'ret_lag_9', 'ret_lag_10', 'ret_lag_11','dividend_be']

# fill missing values with 0
stock_character[na_cols] = stock_character[na_cols].fillna(0)

drop_cols = ['SEC_CODE','TRADE_DATE','INDUSTRYNAME','Book_Value', 'ln_be', 'beta','Asset_YoY','Book_YoY', 'Profitability','past_cumret_12']
...
stock_ret_beta = pd.read_parquet(os.path.join(RESULTS_DIR, "stock_character/stock_beta_before2014"))
stock_character = stock_character.merge(stock_ret_beta[['SEC_CODE','beta_b2014']], how='left')
...
# keep only market returns calculated from stocks listed in SSE
mkt_ret = mkt_ret[mkt_ret['Markettype'] == 1]

mkt_ret = gf.convert_date(mkt_ret,"TRADE_DATE", "TRADE_DATE")

stock_character = stock_character.assign(
    TRADE_MONTH=stock_character['TRADE_DATE'].astype(str).str[:6]
).merge(
    mkt_ret.assign(TRADE_MONTH=mkt_ret['TRADE_DATE'].astype(str).str[:6])[['TRADE_MONTH', 'mkt_ret']],
    on='TRADE_MONTH',
    how='left'
...
stock_character['beta_adj_ret'] = (stock_character['ret']-stock_character['Rf']) - stock_character['beta_b2014']*(stock_character['mkt_ret']-stock_character['Rf'])

### Save Processed Data ###

stock_character.to_parquet(os.path.join(RESULTS_DIR, "stock_character/stock_processed"), index=False)

# === 3. Summary Statistics ===

# Industry Dummy
stock_character = pd.get_dummies(stock_character, columns=['INDUSTRYNAME'], drop_first=False,dtype=np.float64)
...
stock_character = stock_character.groupby(['TRADE_DATE']).apply(lambda month: winsor_month_data(month, winsor_list))

# Drop TRADE_DATE from the index to avoid duplication
if 'TRADE_DATE' in stock_character.index.names: stock_character.index = stock_character.index.droplevel('TRADE_DATE')

# Get cross-sectional z-scores
zscore_cols = ['past_cumret_12', 'beta','ln_be','Profitability_win', 'Asset_YoY_win', 'Book_YoY_win', 'dividend_be_win','ipo_gap']

zscore_cols_name = [x+'_z' for x in zscore_cols]

stock_character[zscore_cols_name] = stock_character.groupby('TRADE_DATE')[zscore_cols].transform(lambda x: stats.zscore(x, nan_policy='omit'))

temp = stock_character.copy()

var_list = ['Market_to_Book','beta','Book_Value', 'Profitability_win', 'Book_YoY_win', 'dividend_be_win','ipo_gap', 'SOE','past_cumret_12']
...
with open(os.path.join(TABLES_DIR, "summary_stat_stock_characteristics.tex"), "w") as f: f.write(summary.to_latex(float_format="%.2f"))

### Save Processed Data ###
stock_character.to_parquet(os.path.join(RESULTS_DIR, "stock_character/stock_processed_win"), index=False)