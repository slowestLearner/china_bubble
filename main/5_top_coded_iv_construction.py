...
stock_character = pd.read_parquet(os.path.join(RESULTS_DIR, "stock_character/stock_processed_win"))
...
def combine_month_data(retail_file, institution_file, keep_cols, stock_character):
    ...
    institution_data = institution_data[institution_data['inside'] == 1] # keep inside assets
    ...
    month_data = pd.merge(month_data, stock_character, on=['SEC_CODE', 'TRADE_DATE'], how='left')
    ...
    month_data['obs_n'] = month_data.groupby(['group_label'])['SEC_CODE'].transform('count')
...
#         return None
    ...
    exog = group[regre_cols+['me_iv1_log', 'me_iv1_log_square', 'me_iv2_log', 'me_iv2_log_square']]
    ...
    if len(missing_industry) >0:
        append_df = {'var_name': missing_industry, 'coef': 0, 't_values': 0,  'p_values':0}
        res_df = pd.concat([res_df, pd.DataFrame(append_df)], ignore_index=True)
        sort_col = ['const'] + controls + ['me_iv1_log', 'me_iv1_log_square','me_iv2_log', 'me_iv2_log_square']
...
#         return None
    ...
    exog = sm.add_constant(exog)
    instrument = group[regre_cols+['me_iv1_log', 'me_iv1_log_square', 'me_iv2_log', 'me_iv2_log_square']] 
    ...
    if top_coded:
        append_df = {'var_name': ['ln_mkv'], 'coef': 0.999, 't_values': 0, 'p_values':0}
...
mkt_ret = pd.read_csv(os.path.join(RESULTS_DIR, "stock_character/mkt_return_month.csv"),parse_dates=['TRADE_DATE'])
...
# define columns used
industry_cols = ['INDUSTRYNAME_采掘', 'INDUSTRYNAME_化工', 'INDUSTRYNAME_钢铁', 
'INDUSTRYNAME_有色金属', 'INDUSTRYNAME_电子', 'INDUSTRYNAME_汽车', 'INDUSTRYNAME_家用电器', 
'INDUSTRYNAME_食品饮料', 'INDUSTRYNAME_纺织服装', 'INDUSTRYNAME_轻工制造', 'INDUSTRYNAME_医药生物', 
'INDUSTRYNAME_公用事业', 'INDUSTRYNAME_交通运输', 'INDUSTRYNAME_房地产', 'INDUSTRYNAME_商业贸易', 
'INDUSTRYNAME_休闲服务', 'INDUSTRYNAME_银行', 'INDUSTRYNAME_非银金融', 'INDUSTRYNAME_综合', 
'INDUSTRYNAME_建筑材料', 'INDUSTRYNAME_建筑装饰', 'INDUSTRYNAME_电气设备', 'INDUSTRYNAME_机械设备', 
'INDUSTRYNAME_国防军工', 'INDUSTRYNAME_计算机', 'INDUSTRYNAME_传媒', 'INDUSTRYNAME_通信'] # take 'INDUSTRYNAME_农林牧渔' as baseline

group_cols = ['SEC_CODE','TRADE_DATE', 'group_label', 'inside', 'HOLD_Value', 'HOLD_BAL', 'w_i_n_raw', 'w_i_0', 'w_i_n_adjust']

exog_cols = ['past_cumret_12','beta','ln_be', 'Profitability_win', 'Book_YoY_win', 'dividend_be_win','ipo_gap', 'SOE'] + industry_cols
...
second_path_reg_summary = os.path.join(RESULTS_DIR, "Regression_Results", reg_version, "IV2SLSsummary")
...
for trade_date in tqdm(trade_dates):
    retail_file = os.path.join(RESULTS_DIR, "group_regre_data_09062022", "retail", f'retail_group_{trade_date}')
    insti_file = os.path.join(RESULTS_DIR, "group_regre_data_09062022", "institution", f'group_insti_{trade_date}')
    month_data = combine_month_data(retail_file, insti_file, group_cols, stock_character)
    group_list = sorted(month_data['group_label'].unique())
    res_first = []
    res_second = []
    for group_label in group_list:
        if group_label.split('_')[0] in ['310','410','420','430','440','450','QFII','RQFII','annuity','social']:
            continue
        group_regre = month_data[month_data.group_label == group_label]
        res_df = group_first_regression(group_regre, exog_cols, trade_date, industry_cols)
        res_first.append(res_df)
        res_df = group_second_regression(group_regre, exog_cols, trade_date, second_path, industry_cols)
        res_second.append(res_df)
    res_first = pd.concat(res_first, ignore_index=True)
    res_first.to_parquet('{}/reg_results_{}'.format(first_path, trade_date), index=False)
    res_second = pd.concat(res_second, ignore_index=True)
    res_second.to_parquet('{}/reg_results_{}'.format(second_path_reg_summary, trade_date), index=False)