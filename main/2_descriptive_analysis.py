...
mkt_ret = pd.read_csv(os.path.join(RESULTS_DIR, "stock_character/market_return_daily.csv"))
mkt_ret['TRADE_DATE'] = pd.to_datetime(mkt_ret['TRADE_DATE'], infer_datetime_format=True) 
...
fig.savefig(os.path.join(FIGURES_DIR, "price_index_full_sample.png"), dpi=300, bbox_inches='tight')
...
xtick = [datetime.date(2014, 7, 1), datetime.date(2015, 1, 1), datetime.date(2015, 6, 12), 
        datetime.date(2016, 1, 31)] #datetime.date(2014, 1, 1),
ax.xaxis.set_ticks(xtick)
ax.xaxis.set_major_formatter(mdates.DateFormatter('%m/%d/%Y'))
fig.savefig(os.path.join(FIGURES_DIR, "price_index_bubble_period.png"), dpi=300, bbox_inches='tight') 

# === 2. Illustrative Individual Stock Returns Examples ===

#### Raw Returns Calculation ###

price_daily = pd.read_parquet(os.path.join(RESULTS_DIR, "stock_character/stock_price_daily_20211130"))
...
price_daily['cum_ret'] = price_daily.groupby(['SEC_CODE'])['ret'].transform(lambda x : (x+1).cumprod()-1) * 100
...
price_daily = price_daily.merge(mkt_ret[['TRADE_DATE', 'mkt_ret']], on=['TRADE_DATE'], how='left')

# import risk-free rate data
risk_free = pd.read_csv(os.path.join(CSMAR_DIR, "stocks_info/TRD_Nrrate.csv"), parse_dates=['Clsdt'])
...
price_daily['beta_adj_ret'] = (price_daily['ret']-price_daily['Rf']) - price_daily['beta_b2014']*(price_daily['mkt_ret']-price_daily['Rf'])

# calculate CAR (cumulative abnormal returns)
price_daily = price_daily.sort_values(by=['SEC_CODE', 'TRADE_DATE'])
price_daily['cum_beta_adj_ret'] = price_daily.groupby(['SEC_CODE'])['beta_adj_ret'].transform(lambda x : (x+1).cumprod()-1) * 100
...
fig.savefig(os.path.join(FIGURES_DIR, "cumulative_retrun_example.png"), dpi=300, bbox_inches='tight')
...
fig.savefig(os.path.join(FIGURES_DIR, "raw_retrun_example.png"), dpi=300, bbox_inches='tight')
