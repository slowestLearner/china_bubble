...
institute_hold = (
    institute_hold.groupby(["TRADE_DATE", "group_label"])["HOLD_Value"]
    .sum()
    .reset_index()
)
...
file_folder = glob.glob(
    os.path.join(
        RESULTS_DIR, "regression_results", "EqualandBook_topcode1", "IV2SLSBeta", "*"
    )
)
...
select_cols = [
    ...
    "INDUSTRYNAME_通信",
    "log_price",
]  # take 'INDUSTRYNAME_农林牧渔' as the base industry
...
beta_all["retail"] = (
    beta_all["group_label"].transform(lambda x: len(x.split("_"))) == 5
).astype("int")
...
beta_bubble["agg_type"] = beta_bubble["group_label"].transform(
    lambda x: x.split("_")[0]
)

beta_bubble["log_BE"] = (
    beta_bubble["log_BE"] + beta_bubble["log_price"]
)  # change the coefficient for log(BE) to log(BE) + log(ME): preference for size. coefficient for log(ME) is unchanged: variable correspondingly become log(ME) - log(BE)
...
for idx, df in enumerate(
    [early_retail, early_insti, mid_retail, mid_insti, late_retail, late_insti]
):
...
with open(os.path.join(TABLES_DIR, "demand_function_parameters.tex"), "w") as f:
...
def gen_value_weighted_quantile(df, q=0.75):
    ...
    for col in df.columns[:-3]:
        ...
        weighted_quantile = df.loc[weighted_quantile_idx][
            ["TRADE_DATE", col]
...
beta_list_retail = gen_value_weighted_coef(
    beta_bubble[beta_bubble.retail == 1][columns]
)
beta_list_institute = gen_value_weighted_coef(
    beta_bubble[beta_bubble.retail == 0][columns]
)

beta_list_retail_q25 = gen_value_weighted_quantile(
    beta_bubble[beta_bubble.retail == 1][columns], q=0.25
)
beta_list_retail_q75 = gen_value_weighted_quantile(
    beta_bubble[beta_bubble.retail == 1][columns], q=0.75
)

beta_list_institute_q25 = gen_value_weighted_quantile(
    beta_bubble[beta_bubble.retail == 0][columns], q=0.25
)
beta_list_institute_q75 = gen_value_weighted_quantile(
    beta_bubble[beta_bubble.retail == 0][columns], q=0.75
)


beta_list_institute = gen_equal_weighted_coef(
    beta_bubble[beta_bubble.retail == 0][columns]
)
...
Regression_Data.loc[
    (Regression_Data["TRADE_DATE"] > 20140700)
    & (Regression_Data["TRADE_DATE"] < 20141299),
    "period",
] = "2Bubble Runup1"
Regression_Data.loc[
    (Regression_Data["TRADE_DATE"] > 20150000)
    & (Regression_Data["TRADE_DATE"] < 20150699),
    "period",
] = "3Bubble Runup2"
Regression_Data.loc[
    (Regression_Data["TRADE_DATE"] > 20150700)
    & (Regression_Data["TRADE_DATE"] < 20160199),
    "period",
] = "4Bubble Crash"
...
Regression_Data.loc[
    (Regression_Data.retail == 0) & (Regression_Data.period == "1Pre Bubble"),
    "type_period",
] = "Institution_1Pre Bubble"
Regression_Data.loc[
    (Regression_Data.retail == 0) & (Regression_Data.period == "2Bubble Runup1"),
    "type_period",
] = "Institution_2Bubble Runup1"
Regression_Data.loc[
    (Regression_Data.retail == 0) & (Regression_Data.period == "3Bubble Runup2"),
    "type_period",
] = "Institution_3Bubble Runup2"
Regression_Data.loc[
    (Regression_Data.retail == 0) & (Regression_Data.period == "4Bubble Crash"),
    "type_period",
] = "Institution_4Bubble Crash"
Regression_Data.loc[
    (Regression_Data.retail == 0) & (Regression_Data.period == "5Post Bubble"),
    "type_period",
] = "Institution_5Post Bubble"

Regression_Data.loc[
    (Regression_Data.retail == 1) & (Regression_Data.period == "1Pre Bubble"),
    "type_period",
] = "Retail_1Pre Bubble"
Regression_Data.loc[
    (Regression_Data.retail == 1) & (Regression_Data.period == "2Bubble Runup1"),
    "type_period",
] = "Retail_2Bubble Runup1"
Regression_Data.loc[
    (Regression_Data.retail == 1) & (Regression_Data.period == "3Bubble Runup2"),
    "type_period",
] = "Retail_3Bubble Runup2"
Regression_Data.loc[
    (Regression_Data.retail == 1) & (Regression_Data.period == "4Bubble Crash"),
    "type_period",
] = "Retail_4Bubble Crash"
Regression_Data.loc[
    (Regression_Data.retail == 1) & (Regression_Data.period == "5Post Bubble"),
    "type_period",
] = "Retail_5Post Bubble"
...
latex_output = latex_output.replace(
    f"\\begin{{tabular}}{{{default_align}}}", f"\\begin{{tabular}}{{{custom_align}}}"
)

with open(
    os.path.join(TABLES_DIR, "demand_function_coefficients_retail_institution.tex"), "w"
) as f:
...
for seq, res in enumerate(res_list[1:], start=1):
    ...
    coef_df = pd.DataFrame(
        {
            "coef": res.params,
            "err": err_series,
            "varname": err_series.index.str.slice(12, -1),
        }
    )
    ...
    for i, mod in enumerate(coef_df.invest_type.unique()):
        ...
        X = base_x + width * i
        ax.bar(
            X, mod_df["coef"], yerr=mod_df["err"], color="none", ecolor=color_list[i]
        )
        # remove axis labels
        ax.set_ylabel("")
        ax.set_xlabel("")
        ax.scatter(
            x=X, marker=marker_list[i], s=120, y=mod_df["coef"], color=color_list[i]
        )
        ...
        ax.set_xticklabels(
            ["Pre-Bubble", "Formation", "Expansion", "Deflation", "Post-Bubble"],
            rotation=0,
            fontsize=16,
        )
    ...
    legend_elements = [
        Line2D(
            ...
            label=coef_df.invest_type.unique()[0],
            color=color_list[0],
            markersize=10,
        ),
        Line2D(
            ...
            label=coef_df.invest_type.unique()[1],
            color=color_list[1],
            markersize=10,
        ),
    ]
    ...
    fig.savefig(
        os.path.join(
            FIGURES_DIR, f"demand_function_retail_institution_{name_list[seq]}.png"
        ),
        dpi=300,
        bbox_inches="tight",
    )
...
for i, mod in enumerate(coef_df.invest_type.unique()):
    ...
    #     ax.xaxis.set_ticks_position('default')
    ax.set_xticklabels(
        ["Pre-Bubble", "Early Run-up", "Late Run-up", "Crash", "Post-Bubble"],
        rotation=0,
        fontsize=16,
    )
    ax.tick_params(axis="y", labelsize=15)
...
fig.savefig(
    os.path.join(FIGURES_DIR, f"demand_elasticity_retail_institution.png"),
    dpi=300,
    bbox_inches="tight",
)
...
Regression_Data["RCP_type"] = Regression_Data["group_label"].transform(
    lambda x: x.split("_")[2]
)  # conditioning on RCP level
Regression_Data["cohort_type"] = Regression_Data["group_label"].transform(
    lambda x: x.split("_")[0]
)  # conditioning on cohort
...
Regression_Data = Regression_Data[
    Regression_Data.TRADE_DATE.between(20110000, 20200000)
]  # conditioining on specific time period

### Wealth Effect ###

Regression_Data["wealth_type"] = "Institution"
Regression_Data.loc[Regression_Data.retail == 1, "wealth_type"] = Regression_Data.loc[
    Regression_Data.retail == 1, "group_label"
...
with open(
    os.path.join(TABLES_DIR, "demand_function_coefficients_wealth.tex"), "w"
) as f:
    f.write(latex_output)
...
for seq, res in enumerate(res_list):
    ...
    #     ax.xaxis.set_ticks_position('default')
    ax.set_xticklabels(
        ["Wealth1(Low)", "Wealth2", "Wealth3", "Wealth4", "Wealth5(High)"],
        rotation=0,
        fontsize=18,
    )
    ...
    ax.set_title(f"Preferences/Beliefs about {name_list[seq]}", fontsize=20)
    fig.savefig(
        os.path.join(FIGURES_DIR, f"demand_function_wealth_{name_list[seq]}.png"),
        dpi=300,
        bbox_inches="tight",
    )
...
Regression_Data.loc[Regression_Data.retail == 1, "cohort_type"] = Regression_Data.loc[
    Regression_Data.retail == 1, "group_label"
...
Regression_Data.loc[Regression_Data.cohort_type == "burst", "cohort_type"] = "4burst"
Regression_Data.loc[Regression_Data.cohort_type == "post", "cohort_type"] = "5post"
Regression_Data.loc[Regression_Data.cohort_type == "Institution", "cohort_type"] = (
    "6Institution"
)

Regression_Data = Regression_Data[
    Regression_Data["TRADE_DATE"].between(20140000, 20170000)
]
...
with open(
    os.path.join(TABLES_DIR, "demand_function_coefficients_cohort.tex"), "w"
) as f:
    f.write(latex_output)
...
for seq, res in enumerate(res_list):
    ...
    ax.set_title(f"Preferences/Beliefs about {name_list[seq]}", fontsize=20)
    fig.savefig(
        os.path.join(FIGURES_DIR, f"demand_function_cohort_{name_list[seq]}.png"),
        dpi=300,
        bbox_inches="tight",
    )
