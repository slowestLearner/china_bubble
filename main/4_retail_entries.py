...
plt.axvline(
    x=pd.to_datetime(20140701, format="%Y%m%d"), color="k", linestyle="--", alpha=0.6
)
plt.axvline(
    x=pd.to_datetime(20150101, format="%Y%m%d"), color="k", linestyle="--", alpha=0.6
)
plt.axvline(
    x=pd.to_datetime(20150630, format="%Y%m%d"), color="k", linestyle="--", alpha=0.6
)
plt.axvline(
    x=pd.to_datetime(20160131, format="%Y%m%d"), color="k", linestyle="--", alpha=0.6
)
...
fig.savefig(
    os.path.join(FIGURES_DIR, "retail_new_entries.png"), dpi=300, bbox_inches="tight"
)
...
retail_hold_stock["circulate_value"] = (
...
netFlow_all["retail"] = list(
    map(lambda x: int(x.startswith("1")), netFlow_all["ACCT_TYPE2"])
)
...
denominator = market_cap_base[
    market_cap_base.SEC_CODE.apply(lambda x: x in big_bubble)
]["circulate_value"].sum()
...
for col in net_flow_agg["retail"].unique():
    ax.plot(
        net_flow_agg[net_flow_agg["retail"] == col]["net_buy_share"], label=f"{col}"
    )
ax.set_xlabel("Time", fontsize=15)
...
fig.savefig(
    os.path.join(FIGURES_DIR, "retail_net_flows.png"), dpi=300, bbox_inches="tight"
)
plt.show()
